...
Required for crypto withdrawals when the account is registered in the European Economic Area (EEA) and the Travel Rule API is enabled.
- **Required fields**: Standard crypto fields + `travelRule` object
- **Wallet types**:
  - `hosted` - Destination is a VASP-hosted wallet (exchange, custodian). Requires `vasp` object.
  - `unhosted` - Destination is a self-custody wallet.
- **Beneficiary types**:
  - `individual` - Personal withdrawals (requires `firstName`, `lastName`)
  - `entity` - Business withdrawals (requires `fullName`)
- **Use when**: The account is registered in EEA countries

<Note>
Get the list of available VASPs via the [Get VASPs endpoint](/api-reference/travel-rule/get-vasps). Use `vaspId` if the destination VASP is in the list, otherwise use `vaspName`.
</Note>

For complete Travel Rule documentation, see the [Travel Rule Overview](/api-reference/travel-rule/overview).

**Example: Individual to hosted wallet (VASP in list)**
```json
{
  "ticker": "BTC",
  "amount": "0.5",
...
**Example: Entity to hosted wallet (VASP not in list)**
```json
{
  "ticker": "USDT",
  "amount": "10000",
...
**Example: Individual to unhosted wallet**
```json
{
  "ticker": "ETH",
  "amount": "2.5",
