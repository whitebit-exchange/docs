...
- For a returning user whose token is still valid, call the endpoint before redirecting again — skipping the redirect when a key already exists avoids a flow the platform rejects. For a first-time user no token exists yet, and WhiteBIT's own authorization page performs the same duplicate check before it issues an authorization code.
