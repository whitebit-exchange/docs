...
sequenceDiagram
    ...
    U->>P: Provide destination address + beneficiary details (walletType, VASP, identity)
    P->>W: POST /api/v4/main-account/withdraw with travelRule object
    Note right of P: travelRule: walletType, beneficiary,<br/>vaspData (vaspId or vaspName)
...
<Warning>
  The legacy flat format is still accepted but **will not pass Travel Rule verification**. Migrate to the structured format above for compliance.
</Warning>
