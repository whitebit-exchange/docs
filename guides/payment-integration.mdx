...
Four scenarios cover the most common partner integrations. Each closes with a link to the tab documenting the endpoints.
...
See the [Crypto Deposits tab](#deposit-address-generation) for endpoint detail.
...
See the [Crypto Withdrawals tab](#create-a-withdrawal) for endpoint detail.
...
See the [Withdrawal with Conversion tab](#conversion-in-the-withdrawal-flow) for endpoint detail.
...
See the [Express Withdraw tab](#express-withdraw-payments) for endpoint detail.
...
<Tabs>
  ...
  <Tab title="Python">
    ```python
    # Generate a fiat deposit invoice (provider value from Asset Status).
    # The send_request() helper is defined in the Crypto Deposits tab.
...
The critical-path call is the estimate — every later step reuses identifiers from its response. The Crypto Deposits tab defines the `send_request()` helper used below.
