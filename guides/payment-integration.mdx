...
- **Webhook URL** — configure the webhook endpoint to receive transaction notifications. See [Webhooks](/platform/webhook) for the setup flow.
...
|-----------|------|----------|-------------|
| `transactionId` | string | Yes | Transaction UUID of the canceled deposit. |
| `address` | string | Yes | Destination wallet address for the refund. |
| `memo` | string | No | Destination [memo](/glossary#memodestination-tag) for the refund address. Without it, the refund goes to the address with no memo. |
...
**Track refund status:** [`POST /api/v4/main-account/refund-history`](/api-reference/account-wallet/get-refund-history) returns every refund on the account with its current status. A refund with status `canceled` is final. A refund with status `in_process` is not final yet, including after a `refund.failed` webhook caused by an internal failure. Filter by `depositId` to check one deposit.
...
| `POST /api/v4/main-account/refund-history` | Track refund status | [Get Refund History](/api-reference/account-wallet/get-refund-history) |
...
- Configure the webhook URL (see [Webhooks](/platform/webhook))
- Verify the HMAC-SHA512 signature on every incoming webhook. Each delivery includes three headers — `X-TXC-APIKEY` (the WhiteBIT webhook API key), `X-TXC-PAYLOAD` (base64-encoded JSON body), and `X-TXC-SIGNATURE` (HMAC-SHA512 of the payload with the webhook key's secret). See [Security Best Practices](/best-practices/security) for the verification algorithm.
