...
Answers to common questions on the WhiteBIT on/off-ramp integration. For the flows and code, see the [Developer guide](/guides/on-off-ramp-guide); for access, onboarding, and positioning, see the [Overview](/guides/on-off-ramp-overview).
...
{/* REVIEW — DEV-55: SEPA EUR deposit-side provider value is not documented in the spec (examples are UAH + VISAMASTER only; a UAH deposit probe returned IBAN_TRANSFER). Confirm the valid SEPA EUR deposit provider value(s) and the hosted-invoice rendering with dev before replacing this hedged answer with a concrete value. */}
<Note>
...
Setting `partialEnable: true` raises the maximum limit for fiat withdrawals. The withdrawal can then complete partially, surfacing as the `Partially successful` status (18). The application must reconcile `requestAmount` against `processedAmount` in the history record and surface the remainder to operations. See the [failure and exception paths](/guides/on-off-ramp-guide#failure-and-exception-paths).
</Accordion>
</AccordionGroup>
...
{/* REVIEW — DEV-57: Outbound webhook delivery IPs are not documented on the portal. Confirm with dev/infra whether the delivery IPs are stable and publishable for allowlisting before adding a concrete list here. */}
<Note>
...
{/* REVIEW — DEV-56: The "near-real-time / under 10 seconds" SEPA Instant claim from the source PDF is not confirmed as platform behavior. Do not add a fixed settlement time; confirm with the account-manager / product owner before publishing any figure. */}
<Note>
...
{/* REVIEW — DEV-58: Whether /convert/estimate|confirm accept fiat tickers (e.g. EUR↔USDT) as the conversion leg is not documented either way (spec examples are crypto-only). Confirm with dev before documenting Convert as a direct fiat↔crypto bridge. */}
<Note>
...
<CardGroup cols={3}>
  ...
  <Card title="Developer guide" icon="code" href="/guides/on-off-ramp-guide">
