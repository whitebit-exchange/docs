...
sidebarTitle: "On/off-ramp integration"
...
- **Completed [institutional onboarding](/institutional/onboarding) with approved fiat access** — a separate phase that is not automatic after KYB (see [Access and onboarding](#access-and-onboarding)).
- **Per-account enablement gates** — the fiat deposit invoice endpoint, dedicated deposit addresses, and Express Withdraw are enabled per account; confirm each with the account manager (see [What the account manager confirms](#what-the-account-manager-confirms)).
- **API access** — [HMAC-signed requests](/api-reference/authentication) on the main account.

## What the integration provides

- **Fiat deposits via API** — `POST /api/v4/main-account/fiat-deposit-url` returns a hosted payment URL for the fiat providers enabled for the account. The documented example uses the VISAMASTER card provider; the deposit-side SEPA bank-transfer provider is confirmed per account during onboarding. See [Fiat operations](/guides/payment-integration#fiat-deposit) in the Payment Integration guide.
{/* TODO: value gated on SME (SEPA) — confirm the deposit-side SEPA bank-transfer provider and flow; only card (VISAMASTER) deposit providers are documented on disk (on-off-ramp-02). */}
...
```
{/* TODO: value gated on SME (SEPA) — confirm the EUR SEPA IBAN withdrawal provider value (SEPA_BCB_GROUP) and whether bankBic is a required beneficiary sub-field for that provider; neither is in the public reference (on-off-ramp-01). */}
