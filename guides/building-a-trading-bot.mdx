...
Build an automated trading bot on the WhiteBIT API. This guide covers the full loop: market data ingestion, order placement, fill monitoring via WebSocket, error handling, reconnection recovery, and a worked grid bot example. All code examples use Python with the `requests` and `websockets` libraries.
...
All private endpoint examples in this guide use the following `send_request` helper for HMAC-SHA512 signing:
...
The kill-switch automatically cancels all active orders after a configurable timeout. This is a critical safety mechanism for unattended bots — if the bot process crashes or loses connectivity, the kill-switch prevents stale orders from executing.
...
The bot must refresh the kill-switch before it expires. If the bot fails to refresh, all orders cancel automatically.
...
Robust error handling is critical for unattended bots. Handle these common scenarios:
...
After reconnecting, the bot must re-authenticate (for private channels) and re-subscribe to all channels. This ensures fresh data snapshots replace any state that went stale during the disconnection.
...
<Warning>
  This grid bot is a teaching example. Before running against real funds: validate all order parameters against the market's `minAmount` and `minTotal` from [`GET /api/v4/public/markets`](/api-reference/market-data/market-info), add proper logging, and test with Demo Tokens (DBTC/DUSDT) on the `DBTC_DUSDT` pair first.
</Warning>
