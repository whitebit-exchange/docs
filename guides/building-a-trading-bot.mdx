...
3. **Order execution** — place orders via REST API (single or bulk), or over the WebSocket connection from September 2, 2026
...
**Endpoint:** [`POST /api/v4/order/new`](/api-reference/trading/create-limit-order)
...
**Endpoint:** [`POST /api/v4/order/bulk`](/api-reference/trading/bulk-limit-order)
...
<Note>
Coming soon — order placement over WebSocket becomes available on September 2, 2026, on the global platform only; the EU platform is not yet supported. Until then, place orders over REST as shown above.
</Note>

The [WebSocket Order Management](/websocket/order-management/overview) methods place, modify, and cancel orders as JSON-RPC calls on the same authorized WebSocket connection used for [fill monitoring](#websocket-fill-monitoring) — an alternative to the REST order endpoints. Placing over the open socket removes the per-request HTTP round trip and the per-request HMAC signature: the connection authorizes once, and every later order frame rides that session. REST `/order/bulk` places up to 20 orders in one request, while the WebSocket methods place one order per frame, so keep REST bulk for atomic batches; the kill-switch deadman below stays on REST.

A placement is one JSON-RPC frame; the server answers with a single frame carrying the same `id`, interleaved with the subscription pushes, so correlate answers by `id`:
...
```json
{
  ...
  "params": [
    { "market": "BTC_USDT", "side": "buy", "amount": "0.01", "price": "60000", "client_order_id": "bot-1" }
  ]
}
```

The transport numbers its errors independently of REST — see the [error reference](/websocket/order-management/overview#error-codes). Order placement requires an authorized connection; the parameter set for each method is on the [Order Management](/websocket/order-management/overview) reference.
...
**Endpoints:**
- [`POST /api/v4/order/kill-switch`](/api-reference/trading/sync-kill-switch-timer) — activate or refresh the timer
- [`POST /api/v4/order/kill-switch/status`](/api-reference/trading/status-kill-switch-timer) — check current timer state
...
async def run_bot():
    ...
    while True:
        ws = await connect_with_retry("wss://wss.whitebit.com/ws")
        try:
...
Call `reconcile_state(MARKET, grid_orders)` after `authenticate_ws(ws)` and before re-subscribing. For a per-fill audit trail, also query [`POST /api/v4/trade-account/executed-history`](/api-reference/trading/query-executed-order-history) to see fills that happened during the disconnect.
...
3. **Clean slate** — cancel leftovers from a previous run with `POST /api/v4/order/cancel/all` scoped to the market. The call removes every open order on the market regardless of type, including manually placed ones.
...
<Note>
  ...
  [`POST /api/v4/trade-account/executed-history`](/api-reference/trading/query-executed-order-history)
  before treating the disappearance as a fill.
</Note>

<Warning>
  Before running against real funds: validate all order parameters against the market's `minAmount` and `minTotal` from [`GET /api/v4/public/markets`](/api-reference/market-data/market-info), and add proper logging. The startup `order/cancel/all` call removes every open order on the configured market regardless of type, including orders placed manually, so run the bot on a dedicated account or sub-account.
</Warning>
