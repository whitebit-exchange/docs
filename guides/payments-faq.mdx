...
EUR fiat deposits use the `SEPA_BCB_GROUP` provider, the same as SEPA withdrawals. That provider value and whether a `bankBic` sub-field is required are confirmed per account and are not documented in the public reference, so verify both with WhiteBIT during onboarding. See the [On/Off-Ramp guide](/guides/on-off-ramp-integration) for the fiat deposit flow.
