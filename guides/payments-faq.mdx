...
Fiat deposits and withdrawals require completed [institutional onboarding](/institutional/onboarding) with approved fiat access — a separate second phase that is not automatic after KYB. See the [On/Off-Ramp guide](/guides/on-off-ramp-integration#access-and-onboarding).

### Per-account activations

Several capabilities are enabled per account rather than by default — the fiat deposit invoice endpoint, dedicated per-user deposit addresses, Express Withdraw, and Travel Rule API access. Request them through the account manager or `institutional@whitebit.com`. See the [enablement gates](/guides/on-off-ramp-integration#access-and-onboarding).
...
{/* TODO: value gated on SME (SEPA) — the deposit-side SEPA provider and the bankBic example for
    EUR fiat deposits. Do not state a provider or worked reference until the payments SME confirms
    it (M8). */}
The deposit-side SEPA provider and the bank-reference fields for EUR fiat deposits are confirmed with WhiteBIT during onboarding. See the [On/Off-Ramp guide](/guides/on-off-ramp-integration) for the fiat deposit flow.
