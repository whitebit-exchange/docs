...
{/* TODO: value gated on SME (SEPA) — the deposit-side SEPA provider and the bankBic example for
    ...
    it (M8). */}
The deposit-side SEPA provider and the bank-reference fields for EUR fiat deposits are confirmed with WhiteBIT during onboarding. See the [On/Off-Ramp guide](/guides/on-off-ramp-guide) for the fiat deposit flow.
