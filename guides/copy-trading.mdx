...
- **Rate limits are per IP address, not per follower.** Every follower served from one backend IP shares a single request budget, so a large fan-out — many independent signed requests from the same host — competes against that one budget rather than each follower having its own. Pace the requests and handle `429` with backoff, and consider spreading a large follower base across egress IPs. See [Rate limits](/api-reference/rate-limits).
...
- **Permission grants vary per follower.** A follower can grant less than the Trade scope. Read the `permissions` array on `GET /oauth2/api-key/info` before mirroring, and skip followers without order-placement permission. External withdrawal is never required for copy trading — do not request it.
...
Subscribe to each follower's [Orders Executed](/websocket/account-streams/orders-executed) and [Deals](/websocket/account-streams/deals) account streams for real-time updates, or poll [Query executed order history](/api-reference/spot-trading/query-executed-order-history) as a fallback. Reconcile each fill to its lead trade and follower with the `clientOrderId` set at placement.
