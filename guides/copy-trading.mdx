...
**Request — [Create Limit Order](/api-reference/trading/create-limit-order) (`POST /api/v4/order/new`):**
...
For market and stop variants, see [Create Market Order](/api-reference/trading/create-market-order) and the other [order endpoints](/api-reference/trading/spot-overview).
...
Full field list: [Create Limit Order](/api-reference/trading/create-limit-order).
...
| Purpose | Endpoint | Reference |
|---------|----------|-----------|
| Place a limit order | `POST /api/v4/order/new` | [Create limit order](/api-reference/trading/create-limit-order) |
| Place a market order | `POST /api/v4/order/market` | [Create market order](/api-reference/trading/create-market-order) |
| Modify an open order | `POST /api/v4/order/modify` | [Modify order](/api-reference/trading/modify-order) |
| Cancel one order | `POST /api/v4/order/cancel` | [Cancel order](/api-reference/trading/cancel-order) |
| Cancel every order on a market for one follower | `POST /api/v4/order/cancel/all` | [Cancel all orders](/api-reference/trading/cancel-all-orders) |
| List a follower's open orders | `POST /api/v4/orders` | [Query unexecuted orders](/api-reference/trading/query-unexecuted-orders) |
...
Common questions from copy-trading partners, beyond the eligibility and gating covered in the [Embedded Trading FAQ](/guides/embedded-trading-faq).
...
`DELETE` is authenticated with a short-lived Bearer access token, so revocation depends on holding a valid token: renew it with the stored refresh token, or obtain a fresh pair through a new consent flow once the refresh token has expired too. A dormant follower's key still winds down without action — an OAuth-issued key auto-deactivates after 14 days without API activity, and is revoked when the follower changes the account password or the account is blocked or frozen. If revoking a dormant follower's key without fresh consent is required, confirm the available management path with WhiteBIT.
...
<CardGroup cols={2}>
  ...
  <Card title="Create Limit Order" icon="file-lines" href="/api-reference/trading/create-limit-order">
