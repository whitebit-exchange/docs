...
<Warning>
Never paste API keys into the AI conversation. Credentials are supplied at connection time — the setup command writes the keys into the AI client configuration, and the client passes the credentials to the server on every connection. The server uses keys only to sign outgoing WhiteBIT API requests — keys are not stored, logged, or cached.
</Warning>
