...
| Detail | Value |
|--------|-------|
| Endpoint | `wss://wss.whitebit.com/ws` |
| Protocol | JSON-RPC 2.0 over WebSocket |
| Inactivity timeout | 60 seconds — send `ping` every 50 seconds or less |
| Rate limit | 12,000 requests per 10 seconds per connection; order placements draw on a separate 10,000 per 10 seconds budget |
...
<Steps>
  <Step title="Connect to the WebSocket endpoint">
    ...
    <Tabs>
      ...
      <Tab title="wscat">
        ```bash
        wscat -c wss://wss.whitebit.com/ws
        ```
      </Tab>
    </Tabs>
...
<CardGroup cols={3}>
  ...
  <Card title="Order Management" icon="pen-to-square" href="/websocket/order-management/overview">
    Place, modify, and cancel orders over the same connection (from September 2, 2026, global platform only).
  </Card>
