...
async def monitor_balance():
    async with websockets.connect("wss://api.whitebit.com/ws") as ws:
        # Authenticate first (see WebSocket Quickstart for auth flow)
...
See the [WebSocket Quickstart](/guides/websocket-quickstart) for authentication and connection setup.
