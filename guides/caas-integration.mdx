...
<Note>
  **Travel Rule affects deposits and withdrawals for EEA and Turkey accounts** — the primary CaaS
  segments (banks, EMIs, fintechs). Crypto withdrawals from those accounts carry a `travelRule`
  object with beneficiary data, and inbound deposits are held until beneficiary verification
  completes (statuses 27 and 28). The Travel Rule API for programmatic verification is enabled per
  account via `institutional@whitebit.com`. See [Travel Rule](/concepts/travel-rule) and
  [Regulatory Compliance](/institutional/compliance).
</Note>
