...
- **Deposit addresses.** Generate a unique address per end user with
  ...
  [Wallet-as-a-Service developer guide](/guides/waas-guide).
...
- **Fiat rails (EUR/SEPA).** Available after Phase 2 onboarding — API-generated deposit invoices and
  SEPA withdrawals. See the [On/Off-Ramp guide](/guides/on-off-ramp-guide).
...
carry a structured payload. <TravelRuleNote /> The Travel Rule API is enabled per account through
...
- **Fast API Key via OAuth.** For flows where end users hold personal WhiteBIT accounts, a partner
  ...
  flow. See [Fast API Key via OAuth](/guides/fast-api-key-via-oauth) and the
  [OAuth overview](/platform/oauth/overview).
- **Trading and conversion.** Spot markets across 750+ pairs, and the Convert service for instant
