...
API keys are supplied at connection time — as `WHITEBIT_API_KEY` and `WHITEBIT_SECRET_KEY` environment variables on the stdio transport, or as `X-WB-Api-Key` and `X-WB-Secret-Key` request headers on the HTTP transport — and are used only to sign outgoing WhiteBIT API requests. No tool takes an API key as a parameter. The MCP server does not store, log, or cache credentials. All processing happens locally — no data is sent to external services beyond WhiteBIT.

Public market data tools (tickers, order book, market info) query public endpoints and need no account credentials.
...
Public market data tools do not require a WhiteBIT account. On the stdio transport these tools answer with no credentials configured, so a market-data-only setup can leave the key variables empty. Example questions:
...
- Never provide API keys in the AI conversation — credentials belong in the AI client configuration that the setup command writes.
