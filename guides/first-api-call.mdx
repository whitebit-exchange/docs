...
Make a first WhiteBIT API call — public data with no authentication, then a private request with HMAC-SHA512 signing. The walkthrough is for developers integrating the v4 REST API for the first time and includes curl and Python examples for each API call.
...
<Steps>
  <Step title="Check server time (public, no auth)">
    Verify connectivity by calling the public server time endpoint — no authentication needed.

    **Endpoint:** `GET https://whitebit.com/api/v4/public/time` ([API Reference](/api-reference/market-data/server-time))
  ...
  <Step title="Fetch market data (public, no auth)">
    Retrieve the list of available markets — spot and futures (TradFi futures: coming soon). The endpoint requires no authentication.

    **Endpoint:** `GET https://whitebit.com/api/v4/public/markets` ([API Reference](/api-reference/market-data/market-info))
  ...
  <Step title="Generate an API key">
    ...
    2. Enable 2FA if not already active — key creation requires active 2FA.
    3. Create a new key with **Info + Trading** permissions.
    4. Optionally whitelist the current IP address (up to 50 addresses per key).
    5. Save the API key and secret immediately — the portal displays the secret only once and offers no way to retrieve it later.

    <Warning>
      Store the API secret securely in an environment variable or secrets manager.
      Never commit secrets to version control or embed credentials in client-side code.
    </Warning>
  </Step>

  <Step title="Authenticated balance check (private)">
    Read the Trade account balance. This call requires HMAC-SHA512 signing.

    **Endpoint:** `POST https://whitebit.com/api/v4/trade-account/balance` ([API Reference](/api-reference/spot-trading/trading-balance))
    ...
    **Expected response:**
    ...
    Each key is an asset ticker. `available` is the amount ready to trade; `freeze` is the amount locked in open orders. An empty response (`{}`) means the Trade balance holds no assets — transfer funds from Main first. See [Balances & Transfers](/concepts/balances) for details on Main, Trade, and Collateral account types.
  </Step>
</Steps>
...
<AccordionGroup>
  <Accordion title="Invalid signature error">
    ...
    - The `nonce` is an integer larger than the previous request's nonce — use Unix milliseconds.
    - The API secret is current — regenerating the key invalidates the old secret.
  ...
  <Accordion title="API key not found or 401 Unauthorized">
    ...
    - Copy the API key exactly — no leading or trailing spaces.
