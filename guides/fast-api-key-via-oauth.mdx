...
- **Copy trading** — the copy-trading platform mirrors lead-trader positions into followers' accounts. Each follower connects a personal WhiteBIT account through the consent screen, and the partner platform places orders with the issued key; execution happens inside the follower's own account.
...
Partners operating accounts *for* customers — a dedicated sub-account per end customer under a partner master account — integrate via the [Broker Program](/guides/broker-guide) instead; the [Integration Paths](/guides/integration-paths) page maps all partner types. Whether [attribution](/guides/waas-guide) or commercial terms apply for own-account partners is confirmed with WhiteBIT during enrollment.

## Prerequisites

<Note>
  Partner enrollment is approval-gated. New partners apply via `institutional@whitebit.com` with the details listed under [How to apply](/guides/integration-paths#how-to-apply); a partner agreement and KYB review are part of onboarding. After approval, technical client setup proceeds via `support@whitebit.com` as described below.
</Note>

- Partner-registered OAuth2 client (`client_id`, `client_secret`, redirect URIs, partner IP allowlist). Contact `support@whitebit.com` to enroll a client; IP allowlist registration is coordinated on the same ticket once the OAuth API key feature is approved for the partner platform. The same channel applies to subsequent IP changes — there is no self-service endpoint today.
