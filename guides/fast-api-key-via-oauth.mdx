...
sidebarTitle: "Fast API Key integration"
...
Partners operating accounts *for* customers — a dedicated sub-account per end customer under a partner master account — integrate via the [Broker Program](/guides/broker-guide) instead; the [Integration Paths](/guides/integration-paths) page maps all partner types. Whether [attribution](/guides/waas-overview) or commercial terms apply for own-account partners is confirmed with WhiteBIT during enrollment.
...
- IP restrictions on the key, when enabled, are the user's own control on their WhiteBIT account — exact-match addresses (ranges and subnets are not supported), applied to trade-API requests. The partner does not manage a key-level IP allowlist; if a user enables the restriction, have them whitelist the partner's egress addresses.
