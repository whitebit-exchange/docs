...
**Emergency cancel:** `POST /api/v4/order/cancel/all` — cancel every open order in a single request. Use the optional `market` field to scope to a single pair and the `type` array to scope to `"spot"`, `"margin"`, or `"futures"`; omit both to cancel everything on the account. This is the "big red button" for live traders; the kill-switch is the deadman timer for unattended operation. `order/cancel/all` is not part of the colocation endpoint subset — call it via the standard public API; the in-zone alternatives are per-order `order/cancel` and the kill-switch. See the [API Reference](/api-reference/spot-trading/cancel-all-orders).
