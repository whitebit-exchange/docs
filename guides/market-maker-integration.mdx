...
- Check status: `POST /api/v4/order/kill-switch/status` — see the [API Reference](/api-reference/trading/status-kill-switch-timer)
...
- **Atomic ladder placement.** `POST /api/v4/order/bulk` places up to 20 limit orders in one request with same-timestamp atomicity. The WebSocket methods place one order per frame — the per-connection budget of 10,000 order placements per 10 seconds absorbs a 20-level requote easily, but the levels land as separate engine events rather than at one timestamp. Refresh a full ladder over REST bulk when same-timestamp placement matters; place and cancel individual quotes over the socket.
...
<CardGroup cols={3}>
  ...
  <Card title="Spot Trading API" icon="square-terminal" href="/api-reference/trading/spot-overview">
