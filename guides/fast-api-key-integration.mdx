...
Every `/oauth2/api-key/*` endpoint is Bearer-authenticated, so none of the three can be called until the partner backend holds an OAuth token for that specific user. The steps below cover the stages that run before that point — client registration, and the consent journey that WhiteBIT hosts on its own pages. The partner writes code only for 0.2 and 0.5; the rest happens on WhiteBIT's side and is observable to the partner as either an authorization code on the callback or no callback at all.
