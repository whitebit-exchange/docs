...
The `bboRole` parameter (available on [`POST /api/v4/order/new`](/api-reference/spot-trading/create-limit-order), [`POST /api/v4/order/bulk`](/api-reference/spot-trading/bulk-limit-order) items, and [`POST /api/v4/order/stop_limit`](/api-reference/spot-trading/create-stop-limit-order)) tells the system to select the best bid/offer price instead of an explicit limit price: `1` = Queue method, `2` = Counterparty method.
