    ...
    <Steps>
      ...
      <Step title="Place a collateral limit order">
        ...
        **Expected response:**
        ...
        When a collateral limit order fills, a position is created. Open positions include a `liquidationPrice` field — monitor the liquidation price for risk management. See the [open positions endpoint](/api-reference/trading/open-positions) for details.
      </Step>
      ...
      <Step title="Cancel the order">
        ...
        To close an open position (after a filled order), use the [close position endpoint](/api-reference/trading/close-position) with `positionId` and `market`:
    ...
    <Steps>
      ...
      <Step title="Check open positions">
        ...
        **Expected response (when a position exists):**
        ...
        Key fields: `basePrice` (entry price), `liquidationPrice` (price triggering forced closure — uses mark price, not last traded price; see [Liquidation](/concepts/liquidation)), `positionSide` (LONG, SHORT, or BOTH in Hedge Mode), `margin` (funds allocated to the position).
      </Step>
