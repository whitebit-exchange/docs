...
- **Up to 100x leverage** — Adjustable per account via the [leverage endpoint](/api-reference/trading/change-collateral-account-leverage). Allowed values: 1, 2, 3, 5, 10, 20, 50, 100.
- **Cash-settled** — profit and loss is the difference between entry and exit price; no physical delivery of the underlying asset
- **Hedge Mode** — Hold simultaneous long and short positions on the same pair ([check mode](/api-reference/trading/collateral-account-hedge-mode), [update mode](/api-reference/trading/update-collateral-account-hedge-mode))
- **270+ perpetual pairs** — BTC_PERP, ETH_PERP, and more
- **Funding rates** — Periodic settlement between long and short holders ([funding history](/api-reference/trading/funding-history))
...
**Directional perpetual trading** — Open leveraged long or short positions on perpetual contracts. Set leverage via the [leverage endpoint](/api-reference/trading/change-collateral-account-leverage), place orders through the [collateral order endpoints](/api-reference/trading/collateral-overview), and monitor positions with the [open positions endpoint](/api-reference/trading/open-positions). The API returns a `liquidationPrice` field in position responses for risk monitoring.

**Delta-neutral strategies** — Use [Hedge Mode](/api-reference/trading/update-collateral-account-hedge-mode) to hold simultaneous long and short positions on the same perpetual pair. Profit from funding rate differentials while maintaining market-neutral exposure. Fetch current funding rates via the [public futures endpoint](/api-reference/market-data/available-futures-markets-list) and historical rates via the [funding history endpoint](/api-reference/trading/funding-history).
...
Check the current margin configuration via the [collateral account summary endpoint](/api-reference/trading/collateral-account-summary). Leverage can be adjusted using the [leverage endpoint](/api-reference/trading/change-collateral-account-leverage).

### Liquidation

Positions face liquidation when the maintenance margin requirement is breached. Liquidation is evaluated against the **mark price**, not the last traded price, and the Maintenance Margin Rate is 3% for leverage 1x–10x. Partial liquidation closes part of the position first; [Auto-Deleveraging](/concepts/auto-deleveraging) is the final stage when order-book liquidity cannot absorb the liquidation order.
Full mechanics — mark price derivation, leverage brackets, the liquidation sequence, and the `liquidationPrice` / `liquidationState` / `marginFraction` fields for risk monitoring: [Liquidation and margin requirements](/concepts/liquidation).

- **Mark price** (not last traded price) is used for liquidation calculations. Temporary price wicks do not trigger liquidation. Subscribe to [Premium index](/websocket/market-streams/premium-index) over WebSocket for a live push of mark price, index price, and funding rate, or fetch a one-time snapshot from the [futures markets endpoint](/api-reference/market-data/available-futures-markets-list).
...
- Fetch the current predicted funding rate for any pair via `GET /api/v4/public/futures`, or subscribe to [Premium index](/websocket/market-streams/premium-index) for a live push
...
| Detail | Value |
|--------|-------|
| Endpoints | 21 shared with Margin (all `POST`, all authenticated) |
...
- **Position Management** — Open positions, ADL quantile (returns perpetual markets only), close position, position history, closed-position PNL, funding history (6 endpoints)

There is no collateral-specific modify endpoint. The shared [modify order](/api-reference/trading/modify-order) endpoint covers collateral markets.
...
For full endpoint documentation, see the [Collateral Trading API Reference](/api-reference/trading/collateral-overview).
...
<CardGroup cols={3}>
  ...
  <Card title="API Reference" icon="square-terminal" href="/api-reference/trading/collateral-overview">
    Full endpoint documentation for all 21 collateral trading endpoints.
  </Card>
</CardGroup>
