...
<Steps>
  <Step title="Check Trade balance">
    ...
    **Expected response:**
    ...
    <Note>
      The cURL examples show request structure with placeholder headers. For a runnable signing implementation, use the Python tab.
    </Note>
  ...
  <Step title="Cancel the order">
    ...
    **Expected response:**
    ...
    <Tip>
      To verify canceled orders later, query the [order history](/api-reference/trading/query-executed-order-history) endpoint.
    </Tip>
  </Step>
</Steps>
