...
<Warning>
Rate limits: `1000` WebSocket connections per minute. Per connection, `10000` order placements and `12000` other JSON-RPC requests per `10` seconds.
</Warning>
...
| Order placements per connection | 10,000 per 10 seconds |
| All other requests per connection | 12,000 per 10 seconds |

Limit WebSocket connection creation to `1000` connections per minute.

Order placements — the `*_place` methods on [Order Management](/websocket/order-management/overview) — count against a budget of `10000` requests per `10` seconds per connection.

Every other JSON-RPC request — subscriptions, one-time queries, order cancellations and modifications, `authorize`, `ping` — counts against a separate budget of `12000` requests per `10` seconds per connection.
...
| Per-channel-type limits | None — the same `12000` requests per `10` seconds budget applies uniformly across all public and private channels |
...
**Batch subscriptions**. Subscribe to all required channels immediately after connecting rather than one at a time. Batching reduces round-trips and keeps connection setup well inside the `12000` requests per `10` seconds budget.
