...
Public `depth` excludes RPI orders by design, so RPI orders never appear in this stream. RPI resting liquidity is not published on any market-data feed — it is visible only in private active orders and in the exchange UI order book.
...
1. **Overlay only orders with `rpi: true`.** Non-RPI orders already appear in the public `depth` aggregate — overlaying every order double-counts the non-RPI orders.
...
const ws = new WebSocket("wss://wss.whitebit.com/ws");
...
public class OrderBookClient {
    ...
    public static void main(String[] args) throws URISyntaxException, InterruptedException {
        ...
        URI uri = new URI("wss://wss.whitebit.com/ws");
