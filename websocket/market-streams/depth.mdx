...
## Including your own RPI orders

Public `depth` excludes RPI orders by design, so they never appear in this stream. RPI resting liquidity is not published on any market-data feed — it is visible only in your private active orders and in the exchange UI order book.

To display your own RPI orders alongside the public book, overlay your private active orders onto the local depth book. Subscribe to [`ordersPending`](/websocket/account-streams/orders-pending) (private) alongside `depth_subscribe`, then merge:

1. **Overlay only orders with `rpi: true`.** Your non-RPI orders already appear in the public `depth` aggregate — overlaying every order would double-count them.
2. **Aggregate by price before merging.** Public `depth` levels are price-aggregated `[price, total amount]`, while the private stream is per-order. Sum your RPI orders by price level, and use each order's `left` (remaining amount), not `amount` (original size).
...
5. **Filter by `market`** to match your depth subscription.

This overlay reflects only your own RPI orders. No feed exposes other participants' RPI liquidity.
