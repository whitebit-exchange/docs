...
Exactly one of `result` and `error` carries meaning. Responses arrive interleaved with subscription pushes, which carry no `id`, so correlate each answer by `id` rather than by arrival order. The same operations are available over REST — see [Create Limit Order](/api-reference/trading/create-limit-order) for the equivalent endpoint family.
