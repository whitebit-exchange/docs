...
The transport limits are per connection: order placements count against a budget of `10000` requests per `10` seconds, and every other JSON-RPC request — cancellations, modifications, subscriptions, `ping` — counts against a separate budget of `12000` per `10` seconds. See [WebSocket Rate Limits](/websocket/rate-limits) for connection limits.
