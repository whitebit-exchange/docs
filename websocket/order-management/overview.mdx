...
The WebSocket transport carries order flow as well as market data and account streams. Twenty-one JSON-RPC methods place, modify, and cancel orders on spot, margin, and futures markets over the same connection used for subscriptions.

This page holds the conventions shared by every order method: the request envelope, value formats, the shared enums, and the complete error reference. Each method page documents only its own request parameters and the behaviour specific to that method.
...
Exactly one of `result` and `error` carries meaning. Responses arrive interleaved with subscription pushes, which carry no `id`, so correlate each answer by `id` rather than by arrival order.

## Decimal values are strings

Every price, amount, and fee travels as a decimal string, which avoids floating-point rounding. The engine normalises trailing zeros on the way out: a price sent as `"1916.40"` returns as `"1916.4"`. Compare decimal values numerically, never by string equality.
...
The numeric `type` values are listed in the [order types](/websocket/account-streams/overview#order-types) table. Two notes on reading the codes: the same code can come from more than one method, because a BBO order becomes an ordinary limit order once it is priced; and a stop order that converts into a market or BBO order carries `price` `"0"` until it activates.
...
| Value | Alias | Behaviour on self-match |
|-------|-------|-------------------------|
...
See [Self-Trade Prevention](/platform/self-trade-prevention) for the platform-level behaviour.
...
| `order types must be an array`, `invalid or duplicate order type` | `order_cancel_all` | Bad `type` array |
| `in post-only mode, only limit orders with the post-only flag are allowed` | Market and stop methods | Market is in post-only mode |
...
| `19` | `market is not margin market` | `collateral_*` | Called on a spot-only market |
| `40` | `order is not post only` | Placement | Market is in post-only mode |
...
| `51` | `trading in the market is not allowed` | All | Market suspended, or the market status blocks the method |
...
The region of the authorized connection decides which trading types — spot, margin, futures — and which individual markets are reachable. On this transport the region is carried by the authorization token; no per-request override exists.
...
The available order types also vary by region. In the EU region the spot order types are restricted and margin and futures are unavailable — see [Order types — Regional restrictions](/concepts/order-types#regional-restrictions).
...
- [Spot Trading API](/api-reference/spot-trading/create-limit-order) — the REST equivalent of these methods
