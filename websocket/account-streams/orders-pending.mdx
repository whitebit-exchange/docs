...
<div style={{borderLeft: '3px solid #60a5fa', paddingLeft: '0.75rem', marginTop: '0.75rem', fontSize: '0.8125rem', color: '#374151', lineHeight: '1.5'}}>
  The private stream returns active orders, including orders using Retail Price Improvement (RPI) mode. Because public `depth` excludes RPI orders, this stream is the source for an account's own RPI orders — see [including RPI orders in a local order book](/websocket/market-streams/depth#including-rpi-orders-in-a-local-order-book) for how to overlay those orders onto the public order book.
</div>
...
- [Query Unexecuted Orders](/api-reference/trading/query-unexecuted-orders) — open order snapshot via REST for state recovery after reconnection.
</RelatedResources>
