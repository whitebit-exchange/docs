...
The engine applies no prioritization by request type (submit, cancel, modify, bulk), order type (limit, market, IOC, post-only), or product (spot versus margin). [Self-Trade Prevention](/platform/self-trade-prevention) and [order type](/concepts/order-types) flags change how an individual order behaves, not where the order sits in the matching sequence.

## Latency factors

Execution latency depends on two factors: the connection point and the endpoint called. Account type and fee tier do not affect latency. The lowest latency is **3–5 ms** via [colocation](/platform/colocation) in Frankfurt, where trading servers connect inside the matching-engine zone. Connections from outside colocation add network transit time proportional to physical distance.
