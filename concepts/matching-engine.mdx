...
Execution latency depends on the connection point and the endpoint called. Fee tier has no effect on matching latency. The lowest latency is **3–5 ms** via [colocation](/platform/colocation), where trading servers connect inside the matching-engine zone. Connections from outside colocation add network transit time proportional to physical distance.
