...
description: "Final-stage liquidation mechanism on WhiteBIT futures markets — closes profitable counterparty positions when standard market liquidity cannot cover a liquidated position."
...
WhiteBIT futures liquidation runs in two stages. The trigger price, maintenance margin schedule, and liquidation sequence that precede both stages are covered in [Liquidation and margin requirements](/concepts/liquidation).
...
## Identifying liquidation and ADL fills

Both the liquidation close order (on the liquidated account) and any ADL fills (on a counterparty account) arrive through the normal execution surfaces. The reliable discriminator differs by delivery channel:

- **WebSocket — [`ordersExecuted`](/websocket/account-streams/orders-executed) stream.** Use the `source` field: `"liquidation"` when the account's own position is being liquidated, `"adl"` when the account's position is closed as the counterparty of another user's liquidation.
- **REST — order and executed history.** The history payloads do not include `source`. Match the `clientOrderId` prefix instead.

|  | Liquidation (account's own position) | ADL (as counterparty) |
|--|--|--|
| `source` (WebSocket) | `"liquidation"` | `"adl"` |
| `clientOrderId` prefix | `liquidation_<orderId>_!` | `adl_<orderId>_!` |
| Order type | Market, `ioc: true` | Post-only limit |
| Terminal status | `FILLED`, or `PARTIALLY_FILLED` if book depth is insufficient | `FILLED` |

<Note>
Do not use `reduceOnly` to identify these fills — it remains `false` on ADL-inserted orders despite the position-reducing effect. When the account's own position is liquidated, the account's other pending orders on that market are swept with the `AUTO_CANCELED_LIQUIDATION` status.
</Note>
...
Negative-Rating positions are still ranked and receive a Grade for the indicator (see [ADL Grade](#adl-grade)), but selection filters those positions out — only positive-Rating positions become ADL counterparties.
...
Fetch the current Grade for open positions via [`POST /api/v4/collateral-account/adl-quantile`](/api-reference/trading/adl-quantile), or subscribe to [ADL quantile](/websocket/account-streams/adl-quantile) over WebSocket for a live push instead of polling.
