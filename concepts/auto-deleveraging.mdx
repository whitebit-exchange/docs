...
- `source`: `"adl"`
- `client_order_id`: `"adl_<order_id>"`
- Terminal status: `FILLED`
- `reduce_only`: `false` — even though the order reduces the counterparty's position

<Note>
Integrators identifying ADL fills in a counterparty account must use the `source` field. The `reduce_only` flag remains `false` on ADL-inserted orders despite the position-reducing effect.
</Note>
