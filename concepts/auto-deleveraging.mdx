...
- **WebSocket — [`ordersExecuted`](/websocket/account-streams/orders-executed) stream.** Use the `source` field: `"liquidation"` when your own position is being liquidated, `"adl"` when your position is closed as the counterparty of another user's liquidation.
- **REST — order and executed history.** The history payloads do not include `source`. Match the `clientOrderId` prefix instead.

|  | Liquidation (your own position) | ADL (as counterparty) |
|--|--|--|
...
<Note>
Do not use `reduceOnly` to identify these fills — it remains `false` on ADL-inserted orders despite the position-reducing effect. When your own position is liquidated, your other pending orders on that market are swept with the `AUTO_CANCELED_LIQUIDATION` status.
</Note>
