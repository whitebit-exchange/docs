---
title: "Order Types"
description: "All 7 order types supported by WhiteBIT, each type's availability across market types, and time-in-force options."
sidebarTitle: "Order Types"
---

WhiteBIT supports 7 order types across spot, margin, and futures markets. Every order operation requires authentication. The REST endpoints listed on this page all use the `POST` method; the same order types become reachable over the WebSocket transport as JSON-RPC methods from September 2, 2026 (global platform only) — see [WebSocket Order Management](/websocket/order-management/overview).
...
Verify availability per market type in the [API Reference](/api-reference/trading/spot-overview).
...
- Spot endpoint: `POST /api/v4/order/market` — [API Reference](/api-reference/trading/create-market-order)
...
- Spot endpoint: `POST /api/v4/order/bulk` — [API Reference](/api-reference/trading/bulk-limit-order)
...
| Endpoint | Path |
|----------|------|
| Collateral limit order | [`POST /api/v4/order/collateral/limit`](/api-reference/trading/collateral-limit-order) |
| Collateral bulk limit order | [`POST /api/v4/order/collateral/bulk`](/api-reference/trading/collateral-bulk-limit-order) |
| Collateral market order | [`POST /api/v4/order/collateral/market`](/api-reference/trading/collateral-market-order) |
| Collateral stop-limit order | [`POST /api/v4/order/collateral/stop-limit`](/api-reference/trading/collateral-stop-limit-order) |
| Collateral trigger market order | [`POST /api/v4/order/collateral/trigger-market`](/api-reference/trading/collateral-trigger-market-order) |
| Collateral OCO order | [`POST /api/v4/order/collateral/oco`](/api-reference/trading/create-collateral-oco-order) |
...
In some regions the set of available order types and execution flags is narrower than the global default. The restriction is applied at order creation: a request for an order type, flag, or BBO role that is not available in the active region is rejected synchronously with HTTP status `453`.
...
These limits currently apply to the **EU** region only. Every other region keeps the full set of order types described above.

### EU region

In the EU region, the available order types are restricted to meet applicable regulatory requirements. As a result:

- **Spot** exposes only `market`, `advanced-limit`, and `stop-market`. On `advanced-limit` the `ioc` flag is forced (`required: true`) and the BBO role is locked to `counterparty` — a maker (post-only / queue) order cannot be placed.
- **`limit`, `multi-limit`, `stop-limit`, and `oco` are not available** on spot.
- **Margin and futures are not available** — requesting order types for these trade types returns an empty list, and order creation is rejected.

### New-listing window

Independent of region, while a market has not yet been fully listed (its trading-open time is still in the future) only a single **post-only limit** order is accepted. Every other order type and flag combination returns `453` until the market is fully listed. This applies because a maker order cannot rest against the empty order book of a not-yet-listed asset.

## Related features

- **Client Order ID** — Attach a custom identifier to orders for tracking. See [Client Order ID guide](/guides/client-order-id).
- **Kill-switch** — Emergency cancellation mechanism that cancels a market's active orders after a configurable timeout, one timer per market. Endpoints: `POST /api/v4/order/kill-switch` (activate) and `POST /api/v4/order/kill-switch/status` (check).
...
<CardGroup cols={2}>
  <Card title="Spot Trading" icon="arrow-right-arrow-left" href="/products/spot/overview">
    Place orders on 800+ spot pairs.
  </Card>
  <Card title="Margin Trading" icon="chart-line" href="/products/margin/overview">
    Leveraged trading up to 10×.
  </Card>
  <Card title="API Reference" icon="square-terminal" href="/api-reference/trading/spot-overview">
