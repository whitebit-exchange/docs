...
WhiteBIT supports 7 order types across spot, margin, and futures markets. Every order operation requires authentication. The REST endpoints listed on this page all use the `POST` method; the same order types are also reachable over the WebSocket transport as JSON-RPC methods — see [WebSocket Order Management](/websocket/order-management/overview).
