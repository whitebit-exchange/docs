...
description: "OAuth 2.0 integration with WhiteBIT enables secure, token-based authorization for third-party applications."
---
...
<Warning>
**Deprecated.** The OAuth 2.0 Authorization Code Grant flow for direct account-data access is deprecated and will be removed on **November 1, 2026**. New partner integrations should use the [Fast API Key flow](/guides/fast-api-key-integration), which issues a WhiteBIT API key on the user's behalf and calls the standard [V4 API](/api-reference/authentication).
</Warning>
...
Enable automated trading strategies without storing WhiteBIT credentials in third-party systems. Trading bots can place, modify, and cancel orders while only having the specific permissions required.

### Portfolio Trackers & Analytics
Allow portfolio management applications to read balance and trade history information without write access to trading functions, providing users with consolidated views of assets across multiple platforms.

### Mobile Applications
Provide a seamless authentication experience in mobile apps without requiring users to enter and store WhiteBIT credentials in the app.
...
**WhiteBIT OAuth 2.0 integration is exclusively offered to close ecosystem partners.** To integrate WhiteBIT OAuth 2.0 with an application, contact support at [support@whitebit.com](mailto:support@whitebit.com) — the channel applies to the deprecated classic flow; new integrations enroll in the [Fast API Key flow](/guides/fast-api-key-integration) via `institutional@whitebit.com`. The support team guides through the OAuth integration process, including:
