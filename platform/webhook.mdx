...
All webhook requests use the POST method with `application/json` content type. The consumer server should respond with a 200 HTTP status code. If the consumer is unable to handle the webhook, the request will be retried every 10 minutes but not more than 5 times.

#### Body data

All webhook requests include the following body data:
...
**method** - string. The name of the method that was evaluated. The Webhook API supports the following methods:

- **code.apply**. Triggered when a code owned by a customer was applied.
...
All requests also contain additional data in headers:
...
Performed when a deposit was accepted. Request example:
...
Performed when a deposit was updated. Request example:
...
Performed when a deposit was canceled. Request example:
...
Performed when a withdrawal was created. Request example:
...
Performed when a withdrawal is pending. Request example:
...
Performed when a withdrawal was canceled. Request example:
...
Performed when a withdrawal was completed. Request example:
