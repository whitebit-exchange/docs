...
This endpoint supports **8 different withdrawal scenarios**. Each scenario requires different fields depending on the currency type, destination, and regulatory requirements. Use the example selector above to switch between scenarios.
...
For cryptocurrencies that exist on multiple blockchains.
- **Required fields**: Same as above + `network`
- **Networks**: ERC20 (Ethereum), TRC20 (Tron), BEP20 (BSC), OMNI, etc.
- **Use when**: Withdrawing tokens that exist on multiple chains

<Warning>
**Critical**: Ensure the `network` matches your destination address type. Sending ERC20 USDT to a TRC20 address will result in **permanent loss of funds**. Verify the network on [Asset Status endpoint](/public/http-v4/asset-status-list).
</Warning>

#### **Scenario 3: Travel Rule Compliance (EEA Users)**
Required for crypto withdrawals if you're from the European Economic Area (EEA).
...
- **Use when**: Your account is registered in EEA countries

### 🔹 Fiat Withdrawals (KYC Required)

<Note>
All fiat withdrawals require KYC verification. Ensure your account is verified before attempting fiat withdrawals.
</Note>
...
- **Use when**: Withdrawing larger amounts that might be processed partially
- **Important**: Your application must handle "Partially successful" withdrawal status
...
<Tip>
The `amount` field in this endpoint **includes** the withdrawal fee. The recipient receives `amount - fee`.
</Tip>

### How it works:

**Example with this endpoint** (`/withdraw`):
- Your balance: 100 USDT
...
<Note>
To specify the exact amount the recipient should receive (with fee added on top), use the [withdraw-pay endpoint](/private/http-main-v4/create-withdraw-request-with-specific-amount) instead.
</Note>

Check current fees via [Get Fees endpoint](/private/http-main-v4/get-fees) before creating withdrawals.
...
**Validation happens on our side**, but double-check addresses before sending.
...
<Warning>
Missing a required memo will result in **permanent loss of funds** or significant delays in recovery. Check currency requirements on [Asset Status endpoint](/public/http-v4/asset-status-list).
</Warning>

### ⚠️ Provider Selection (Fiat)

Fiat withdrawals require the correct provider:
- Check available providers via [Asset Status endpoint](/public/http-v4/asset-status-list)
...
```
1. Check your balance: GET /api/v4/main-account/balance
...
This means validation succeeded and the withdrawal process has started. The withdrawal is now queued for processing.
...
Search by your `uniqueId` to track the transaction.

**Withdrawal status codes**:
- **Successful**: 3, 7
- **Canceled**: 4, 9  
- **Pending**: Other codes (see [history endpoint](/private/http-main-v4/get-deposit-withdraw-history) for full list)
...
<Tip>
**Default network**: If you don't specify a `network` parameter, the default network is used (typically ERC20 for USDT). Always specify the network explicitly to avoid confusion.
</Tip>
...
- ✅ Amount is within your balance minus fees
...
- [Create withdraw request with specific amount (fee not included)](/private/http-main-v4/create-withdraw-request-with-specific-amount) - Alternative endpoint where fee is added on top
- [Get deposit/withdraw history](/private/http-main-v4/get-deposit-withdraw-history) - Monitor withdrawal status
- [Get fees](/private/http-main-v4/get-fees) - Check current withdrawal fees
- [Main balance](/private/http-main-v4/main-balance) - Check available balance before withdrawal
- [Asset Status endpoint](/public/http-v4/asset-status-list) - Verify currency can be withdrawn and get network/provider info
