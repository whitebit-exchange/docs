...
- **Auto-deactivation:** API keys that remain unused for **14 consecutive days** are automatically deactivated. An IP whitelist does not exempt a key from this rule
- **Reserve keys:** Send one authenticated request from each standby key every 14 days to keep it active — any authenticated endpoint resets the timer
- **Rotation strategy:**
...
3. Compute `HMAC_SHA512(base64_encoded_body, api_secret)`, where `api_secret` is the secret of the webhook key
