...
- [ ] Bulk endpoints used where available (e.g., [`/order/bulk`](/api-reference/trading/bulk-limit-order) for up to 20 orders per call).
...
- [ ] Webhook URL configured (see [Webhooks](/platform/webhook)).
...
- [ ] Kill-switch configured for trading applications — one timer per traded market; cancels that market's open orders after a timeout if the system becomes unresponsive.
...
3. **Verify authentication:** Make an authenticated call to [`POST /api/v4/trade-account/balance`](/api-reference/trading/trading-balance). If the response is `401`, debug the HMAC signing implementation before proceeding.
