...
If the order amount exceeds the current position size, the system automatically reduces the order to match. This applies on placement and execution.
...
If a position closes while a reduce-only [limit order](#limit-order) is still pending, the backend auto-cancels the order with status `AUTO_CANCELED_REDUCE_ONLY`.

**Availability**

Coming soon — available on collateral (margin and futures) markets only. See [Order Types — Reduce-only orders](/concepts/order-types#reduce-only-orders) for details and endpoint links.
</Accordion>
</AccordionGroup>
