...
Anti-Money Laundering. Controls and checks to detect and prevent money laundering. WhiteBIT runs AML checks on deposit and withdrawal flows.
</Accordion>
...
In Model B (customers' own accounts), the mechanism that links an end user's WhiteBIT account and activity to the partner that onboarded the user, so fee-share and reporting apply without the partner holding the account. See [Integration models](/concepts/integration-models).
</Accordion>
...
<AccordionGroup>
<Accordion title="B2B partner account">

An account designation for business-to-business (B2B) institutional partner services. Certain endpoint groups require a B2B partner account — for example fixed [lending](/products/lending/overview) plans and the [Travel Rule](#travel-rule) API — and canceled orders are not recorded in order history. Request access through the [institutional services form](https://whitebit.com/institutional-services/b2b).
</Accordion>
...
<Accordion title="Demo tokens (DBTC/DUSDT)">

Test assets for risk-free Spot trading on the live platform. WhiteBIT has no public testnet — activate free demo tokens (0.5 DBTC + 1,000 DUSDT) from the [WhiteBIT Codes page](https://whitebit.com/codes) with no KYC required, and trade the `DBTC_DUSDT` pair through the standard API endpoints. Demo tokens support Spot trading only.
</Accordion>
...
<Accordion title="Express Withdraw">

A gated B2B payment feature that charges a specific amount from a WhiteBIT user's [Main balance](#balance-main) to the partner's Main balance through a signed, single-use payment token valid for 90 seconds. The transfer settles instantly, off-chain, and without a [fee](#fee). See the [Payment Integration guide](/guides/payment-integration#express-withdraw-payments).
</Accordion>
</AccordionGroup>
...
<AccordionGroup>
<Accordion title="Grid trading">

An automated strategy that places [limit orders](#limit-order) at fixed price intervals around a center price. When a buy order fills, the strategy places a sell order one grid level above; when a sell order fills, the strategy places a buy order one level below. Profit comes from price oscillation within the grid range. See the [Trading Bot guide](/guides/building-a-trading-bot) for the strategy walkthrough.
</Accordion>
</AccordionGroup>
...
<AccordionGroup>
<Accordion title="Hedge mode">

An account-level setting for collateral trading accounts that allows simultaneous long and short [positions](#position-side) on the same market; when disabled, the account operates in one-way mode. Toggling requires zero open positions. See [Collateral account hedge mode](/api-reference/trading/collateral-account-hedge-mode) and [Update collateral account hedge mode](/api-reference/trading/update-collateral-account-hedge-mode).
</Accordion>
...
<Accordion title="HMAC-SHA512">

A keyed hash algorithm that signs private REST API requests: the client computes `hex(HMAC_SHA512(payload, key=api_secret))` and sends the result in the `X-TXC-SIGNATURE` header. See [Authentication](/api-reference/authentication) for the full signing scheme.
</Accordion>
</AccordionGroup>
...
Restricting an API key to accept requests only from pre-approved IP addresses, with up to 50 addresses per key. Configure the whitelist at the API settings page. See [Security Best Practices](/best-practices/security) for key-management guidance.
</Accordion>
</AccordionGroup>
...
A function to cancel all [orders](#orders) on a specific market automatically when the timer expires. Each timer covers a single market and can be restricted to spot, margin, or futures order types.
</Accordion>

<Accordion title="KYB">

Know Your Business. Verification of a business entity's identity and ownership, the corporate counterpart of [KYC](#kyc). Part of [Institutional Onboarding](/institutional/onboarding).
</Accordion>

<Accordion title="KYC">

Know Your Customer. Identity verification of an individual customer before trading or withdrawing.
</Accordion>
</AccordionGroup>
...
<Accordion title="MiCA">

Markets in Crypto-Assets. The European Union's regulatory framework for crypto-asset service providers. See [Regulatory Compliance](/institutional/compliance).
</Accordion>

<Accordion title="Model A">

The partner-operated sub-account model: the partner creates and manages one WhiteBIT sub-account per end user under a master account and holds the sub-account API key. Used by account operators and managed-trading platforms. See [Integration models](/concepts/integration-models).
</Accordion>

<Accordion title="Model B">

The customers'-own-accounts model: each end user holds a standalone WhiteBIT account and the partner connects through a [Fast API Key](#fast-api-key); WhiteBIT runs the KYC, and activity is linked to the partner by [attribution](#attribution). See [Integration models](/concepts/integration-models).
</Accordion>
...
<Accordion title="Omnibus account">

A single pooled account holding multiple end users' funds together. WhiteBIT does not offer a pooled omnibus model for partner integrations — pooled custody conflicts with MiCA client-fund segregation and Travel Rule requirements, so each model keeps segregated sub-accounts. See [Integration models](/concepts/integration-models).
</Accordion>
...
Profit and loss on a position. `grossPnl` is the realized profit and loss before funding, with trading fees already deducted; `netPnl` is `grossPnl` minus total funding. See [Closed-position PNL](/api-reference/trading/closed-position-pnl).
</Accordion>
...
The order-matching rule used by the [matching engine](#matching-engine): better-priced [orders](#orders) fill first, and orders resting at the same price fill in the sequence of arrival at the engine.
</Accordion>
...
RPI executions apply a specific [fee](#fee) or rebate model defined for the account. When a user trades through a [sub-account](#sub-account), the system applies the effective sub-account fee configuration. Use [`Query Market Fees`](/api-reference/trading/query-market-fee) to retrieve account-wide fees, or [`Query Market Fee For A Single Market`](/api-reference/trading/query-market-fee-single) to retrieve the effective fee for one market.
</Accordion>

<Accordion title="Retail flag">

The `retail` request parameter on the [Create Limit Order](/api-reference/trading/create-limit-order) and [Bulk Limit Order](/api-reference/trading/bulk-limit-order) endpoints marks a [limit order](#limit-order) as a retail-source taker eligible to match against orders submitted by [Retail Price Improvement (RPI)](#retail-price-improvement-rpi) makers. The Retail flag is the taker-side counterpart to the `rpi` flag, which marks the maker side of the same feature.
...
**Combination with RPI**

The Retail flag can be sent together with the `rpi` flag. An order with both `retail=true` and `rpi=true` is an RPI maker and matches exactly like an order with only `rpi=true`: the Retail flag has no effect on it, as on any post-only order. See [Order Parameter Rules](/guides/order-parameter-rules) for the complete list of unsupported parameter combinations.
</Accordion>
...
Crypto deposits are a separately enabled capability and are disabled by default. Once enabled for the account, the capability applies to the account and its sub-accounts; to request it, contact the assigned Account Manager or email institutional@whitebit.com.

A sub-account can use a custom trading [fee](#fee) or rebate model defined for the sub-account. Use [`Query Market Fees`](/api-reference/trading/query-market-fee) to retrieve effective fees when an API key belongs to a sub-account.

[RPI](#retail-price-improvement-rpi) trading uses account-specific fees or rebates. When a user places RPI orders through a sub-account, the system applies the effective sub-account fees. Use [`Query Market Fees`](/api-reference/trading/query-market-fee) to verify fees for RPI executions.
</Accordion>
</AccordionGroup>
...
Traditional-finance (TradFi) futures contracts on assets such as equities (for example, `AAPL_USD`). These contracts carry `type: tradfiFutures` and `isTradFiFutures: true` in the [markets](/api-reference/market-data/market-info) response, and availability is region-gated. Placing or modifying an order on a TradFi market additionally requires the account to hold the TradFi futures permission — see [Markets & Trading Pairs](/concepts/markets) for both gates. See [Perpetual futures](#perpetual-futures) for crypto-settled contracts.
</Accordion>

<Accordion title="Travel Rule">

A regulatory requirement for Virtual Asset Service Providers (VASPs) to share originator and beneficiary information for cryptocurrency transfers above certain thresholds, mandated by frameworks such as the Financial Action Task Force (FATF) recommendations and Markets in Crypto-Assets (MiCA). WhiteBIT can hold deposits under statuses 27 (awaiting verification) and 28 (confirmation in progress) until verifying the required data. See [Travel Rule](/concepts/travel-rule) for when the rule applies and the API workflow.
</Accordion>
</AccordionGroup>
...
An HTTP callback the platform sends to a partner-configured URL when an event occurs, such as a deposit or withdrawal status change. WhiteBIT signs each delivery so the consumer can verify the origin. See [Webhooks](/platform/webhook) for setup, the event catalog, and retry behavior.
</Accordion>
