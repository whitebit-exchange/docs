...
Traditional-finance (TradFi) futures contracts on assets such as equities (for example, `AAPL_USD`). These contracts carry `type: tradfiFutures` and `isTradFiFutures: true` in the [markets](/api-reference/market-data/market-info) response, and availability is region-gated. Coming soon: placing or modifying an order on a TradFi market will additionally require the account to hold the TradFi futures permission — see [Markets & Trading Pairs](/concepts/markets) for both gates. See [Perpetual futures](#perpetual-futures) for crypto-settled contracts.
</Accordion>
