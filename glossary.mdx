...
RPI executions apply a specific [fee](#fee) or rebate model defined for the account. When a user trades through a [sub-account](#sub-account), the system applies the effective sub-account fee configuration. Use the `Query Market Fees` endpoint to retrieve effective fees.
</Accordion>
...
<Note>Coming soon: not yet available or returned by the API.</Note>

Traditional-finance (TradFi) futures contracts on assets such as equities (for example, `AAPL_USD`). Once available, these contracts will carry `type: tradfiFutures` and `isTradFiFutures: true` in the [markets](/api-reference/market-data/market-info) response, and availability will be region-gated. See [Perpetual futures](#perpetual-futures) for crypto-settled contracts.
</Accordion>
