...
Traditional-finance (TradFi) futures contracts on assets such as equities (for example, `AAPL_USD`). These contracts carry `type: tradfiFutures` and `isTradFiFutures: true` in the [markets](/api-reference/market-data/market-info) response, and availability is region-gated. See [Perpetual futures](#perpetual-futures) for crypto-settled contracts.
</Accordion>
