...
<Accordion title="Lending (Smartplan) endpoint 403 access errors">
Lending endpoints require B2B partner permissions. This applies to both Fixed (`/api/v4/main-account/smart/`) and Flex (`/api/v4/main-account/smart-flex/`) endpoints. Until access is granted, these endpoints return `403` with the message `You don't have permission to use this endpoint. Please contact support for more details`. To gain access:

- Fill the institutional services form at [https://whitebit.com/institutional-services/b2b](https://whitebit.com/institutional-services/b2b)
...
WhiteBIT offers colocation across EEA/EMEA and APAC regions. Colocation provides access to a subset of 35 trading endpoints. See [Colocation](/platform/colocation) for infrastructure details and the [Market Maker Guide](/guides/market-maker-integration) for the full endpoint list.
</Accordion>
</AccordionGroup>
...
The kill-switch automatically cancels all active orders after a configurable timeout. Activate via `POST /api/v4/order/kill-switch` with a `timeout` parameter (in seconds). Use `POST /api/v4/order/kill-switch/status` to check the current state. Designed as a safety mechanism for automated trading strategies. See the [Market Maker Guide](/guides/market-maker-integration) for usage patterns.
</Accordion>
...
<Accordion title="API key auto-deactivation after inactivity">
The platform automatically deactivates an API key that receives no requests for 14 consecutive days. An IP whitelist does not exempt a key from this rule — whitelisted and non-whitelisted keys deactivate on the same schedule. Reactivate a deactivated key at [whitebit.com/settings/api](https://whitebit.com/settings/api). See [Key rotation](/best-practices/security#key-rotation).
</Accordion>

<Accordion title="Keeping spare API keys active">
Send at least one authenticated request from each spare key every 14 days. Any authenticated endpoint resets the inactivity timer — a balance check via `POST /api/v4/main-account/balance` is sufficient. Keys held in reserve without periodic traffic deactivate on the same schedule as unused keys. See [Main balance](/api-reference/account-wallet/main-balance).
</Accordion>

<Accordion title="Reasons an API key becomes disabled">
An API key becomes disabled after 14 consecutive days without requests, or when deactivated from the account settings. Check the key status at [whitebit.com/settings/api](https://whitebit.com/settings/api), resolve the cause, then re-enable the key. See [Security best practices](/best-practices/security).
</Accordion>
</AccordionGroup>
...
For details, see the [WebSocket API](/websocket/market-streams/overview). For rate limit tiers and endpoint-specific limits, see [Rate Limits](/api-reference/rate-limits). For colocation infrastructure, see the [Market Maker Guide](/guides/market-maker-integration).
</Accordion>
...
Official SDKs exist for Go, Python, and PHP. See the [SDKs page](/sdks) for installation instructions and GitHub links. For raw HMAC-SHA512 signing without an SDK, see [First API Call](/guides/first-api-call).
</Accordion>
...
Use the REST API for one-time request–response calls: checking balances, fetching trade history, and placing orders. Use the WebSocket API for real-time streaming data: live orderbook updates, trade feeds, and balance changes. From September 2, 2026, order placement, modification, and cancellation are also available over WebSocket as an alternative transport, on the global platform only — the [order methods](/websocket/order-management/overview) run on the same authorized connection as the streams, which suits low-latency placement without opening and re-authenticating a connection per request. REST polling at high frequency consumes rate limits — switch to WebSocket for data that updates continuously. See [Rate Limits](/api-reference/rate-limits) for REST constraints, [WebSocket Rate Limits](/websocket/rate-limits) for the per-connection WebSocket request budgets, [WebSocket Quickstart](/guides/websocket-quickstart) for streaming setup, and [Order Management](/websocket/order-management/overview) for the WebSocket order methods.
</Accordion>

<Accordion title="Choosing the right SDK">
Official SDKs are available for Go, Python, and PHP. Check the [SDKs page](/sdks) for current version, maintenance status, and whether each SDK handles HMAC signing automatically.
</Accordion>

<Accordion title="API access levels and prerequisites">
Public endpoints (market data, tickers, orderbook) require no account or authentication. Private endpoints (trading, account management) require API keys — create these keys in account Settings. Institutional features (colocation, OTC, Embedded Trading) require [institutional onboarding](/institutional/onboarding). See [First API Call](/guides/first-api-call) for the recommended integration path.
</Accordion>
</AccordionGroup>
...
WhiteBIT offers Market-Making Programs, Embedded Trading (up to 40% fee share), Sub-Account management, OTC trading, Crypto Lending for business, colocation, and Liquidity Provision. See the [Institutional Overview](/institutional/overview) for the full catalog.
</Accordion>
...
Base trading fees are 0.1% maker / 0.1% taker. The Market-Making Program offers rebates up to -0.012% maker. Embedded Trading provides up to 40% fee share on referred user trading fees; the applicable rate is agreed individually during enrollment. Contact institutional@whitebit.com for volume-based arrangements.
</Accordion>
...
WhiteBIT holds licenses in multiple EU jurisdictions and maintains ISO 27001, PCI DSS, and CCSS security certifications, with the CCSS Level 3 assessment conducted by Hacken. GDPR compliance is maintained for data processing. For specific VASP authorization details and jurisdiction information, contact institutional@whitebit.com. See [Regulatory Compliance](/institutional/compliance).
</Accordion>

<Accordion title="Colocation for low-latency trading">
Colocation is available across EEA/EMEA and APAC regions. Access is limited to a subset of 35 trading endpoints. See [Colocation](/platform/colocation) for setup details and the [Market Maker Guide](/guides/market-maker-integration) for the full endpoint list.
</Accordion>

<Accordion title="Sub-account permissions not applied after creation">
A sub-account created with its own email address (dedicated KYC, `shareKyc: false`) must have that email confirmed before permissions can be configured. The end customer confirms it by opening the personal link from [`POST /api/v4/sub-account/kyc-url`](/api-reference/sub-accounts/kyc-url) and completing verification, which covers both identity verification and email confirmation.

Permissions configured before that confirmation are not retained. Re-apply the permission set with [`POST /api/v4/sub-account/edit`](/api-reference/sub-accounts/edit-sub-account) once the sub-account is active, and send the full `permissions` object on every edit: flags omitted from an edit request are turned off without a warning.

A sub-account created with `shareKyc: true` accepts permissions immediately at creation. See the [Sub-account integration guide](/guides/sub-account-integration) for the full flow.
</Accordion>
</AccordionGroup>
...
Since May 19, 2025, crypto withdrawals from EEA accounts require a `travelRule` object in the withdrawal request body. The object uses a structured format: `walletType` (hosted or unhosted destination), `beneficiary` (receiver details), and `vaspData` (destination platform identification). See [Regulatory Compliance](/institutional/compliance) for the field reference and code examples.
</Accordion>

<Accordion title="Security certifications and audits">
WhiteBIT holds ISO 27001, PCI DSS, and CCSS security certifications, with the CCSS Level 3 assessment conducted by Hacken. GDPR is a regulation (not a certification) — WhiteBIT processes data in accordance with GDPR requirements. For audit reports or verification details, contact institutional@whitebit.com.
</Accordion>
</AccordionGroup>
...
<CardGroup cols={2}>
  ...
  <Card title="Partner Solutions" icon="signs-post" href="/guides/partner-solutions">
    Find the recommended integration path for market makers, account operators, bot platforms, wallet backends, payment providers, prop firms, and institutional clients.
  </Card>
</CardGroup>
