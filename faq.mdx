...
WhiteBIT does not offer a public sandbox or testnet. Test integrations using minimum order sizes on the live API, placing limit orders at prices far from the current market price to avoid fills. Cancel test orders after verification.
</Accordion>
...
WhiteBIT does not offer a public sandbox or testnet environment. Test all integration flows on the live API using minimum order sizes. Use `GET /api/v4/public/markets` to find pairs with the lowest minimums. See the [Go-Live Checklist](/best-practices/go-live-checklist) for safe testing guidance.
</Accordion>
