...
Each futures market entry now returns the bounds the funding rate can reach: `funding_cap` (upper) and `funding_floor` (lower). Use the fields to model worst-case funding costs in trading algorithms instead of hardcoding exchange-wide limits. Both fields are decimal strings served from the last successfully refreshed funding snapshot, so a temporary source outage does not drop the values; both return `null` only when no snapshot has been loaded since service start.
...
<CardGroup cols={2}>
  ...
  <Card title="Funding history" href="/api-reference/collateral-trading/funding-history" icon="clock-rotate-left" horizontal/>
</CardGroup>
</Update>
