...
Each futures market entry will carry `mark_price` — the fair value of the perpetual contract, derived from the underlying index price and the futures basis. Mark price, not `last_price`, is what liquidation and auto-deleveraging are evaluated against, so reading it lets an integration reproduce the distance to liquidation the platform itself measures instead of approximating it from the order book. The value is a decimal string from the latest premium index snapshot and is an empty string while no fresh snapshot is available for the market.

The same release repoints `index_price` and `funding_rate` to that premium index source. Field names and the decimal-string format are unchanged, so no integration change is required. `index_price` now carries the index the contract actually tracks, falling back to the last traded price of the corresponding spot market while the index is unavailable.
