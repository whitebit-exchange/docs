...
**New `tickSize` and `stepSize` fields on `GET /api/v4/public/markets`**

Every market entry now carries `tickSize` — the minimal price step a limit order price must be a multiple of — and `stepSize` — the minimal quantity step an order amount must be a multiple of. Both are decimal strings passed through verbatim from the trade engine, so the values follow a precision change on the market automatically. Use the fields for pre-trade validation of order price and amount instead of deriving steps from `moneyPrec`/`stockPrec`. Either field is an empty string when the engine does not provide the value for the market, so handle the empty string before parsing.
