...
`POST /api/v4/trade-account/order/history` accepts `MAINTENANCE` as the `status` filter and returns `maintenance` in the response. The value is the one lowercase entry in an otherwise uppercase status set. Maintenance cancellations are no longer reported as `CANCELED`, so the `CANCELED` filter excludes maintenance records.

`POST /api/v4/trade-account/order/history/query` accepts `maintenance` as the `status` filter and returns the same value in the response `status` field. Maintenance records carry an empty `delistingKind`, so the `status` field is the discriminator, and a request that omits `status` returns maintenance records next to the delisting records.
