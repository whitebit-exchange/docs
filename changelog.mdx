...
<Note>
Coming soon: these fields are not yet returned by the endpoint.
</Note>

Once live, the Query Market Fees response will return the RPI maker fee premium, an additional fee component applied on top of the maker rate when a Retail Price Improvement (RPI) order executes. `rpi_maker_fee_premium` will cover spot and margin markets; `futures_rpi_maker_fee_premium` will cover futures markets. The effective RPI maker rate equals the base maker rate plus the applicable premium. Both fields return `null` when the RPI order mode is not enabled for the account or when no premium is configured.
